...
let
  ...
  clumsyLinuxStdenvStages =
    ...
    import (self + /pkgs/stdenv/linux) args;
  ...
  clumsyCrossLinuxStdenvStages =
    ...
    init bootStages ++ [
      ...
      (buildPackages: {
        ...
        ++ (if crossSystem.isWasm then [ (import (self + /pkgs/top-level/static.nix)) ] else [ ]);
        selfBuild = false;
  ...
  topLevelFn = let inherit determineStdenvStages; in
    ...
    let
      ...
      configEval = evalModules {
        modules = [
          (self + /pkgs/top-level/config.nix)
          ({ options, ... }: {
      ...
      allPackages = newArgs:
        import (self + /pkgs/top-level/stage.nix) ({ inherit lib nixpkgsFun; } // newArgs);

      boot = import (self + /pkgs/stdenv/booter.nix) { inherit lib allPackages; };
